  ...
  .then((templateInfo) => Promise.resolve()
    .then(() => {
      ...
      if (semver.lt(appVersion, templateInfo.minimumChromelessVersion)) {
        return Promise.reject(new Error('Chromeless is outdated. Please update Chromeless first to continue.'));
      }
                ...
                if (currentTime - lastUpdated > 2000) {
                  process.send({
                    progress: {
                      ...
                      desc: `Downloading template ${templateInfo.version} (${formatBytes(downloadedLength)}/${formatBytes(totalLength)})...`,
                    },
                  });
